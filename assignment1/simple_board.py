...
class SimpleGoBoard(object):
    ...
    def check_win(self):
        """
        Check if there is a five in the row.
        ---------
        board: numpy array, filled with BORDER
        """
        ...
        directions = [- self.NS, - self.NS + 1, 1, self.NS + 1]
        start = self.row_start(row=1)
        for pnt in range(start, len(self.board)):
            if self.board[pnt] == EMPTY or self.board[pnt] == BORDER:
                continue

            color = self.board[pnt]
            # Check if there exist 5 in a line in any direction
            for direct in directions:
                pntNext = pnt + direct
                count = 1
                for _ in range(1, 5):
                    if self.board[pntNext] != color:
                        break
                    pntNext += direct
                    count += 1
                
                # if 5 in a line, return color
                if count == 5:
                    return color
