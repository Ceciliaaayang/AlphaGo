...
import numpy as np
import re
from simple_board import callAlphabeta
import time
import signal
...
class GtpConnection():
    ...
    def genmove_cmd(self, args):
        ...
        # try to use solver firstly
        response, result = self.solver()
    ...
    def timelimit_cmd(self, args):
        ...
        if(timelimit >= 1 and timelimit <= 100):
            # set time limit into board
            self.timelimit = timelimit
            self.respond()
        else:
            raise ValueError("time should be in [1, 100]")
    ...
    def solver(self):
        ...
        playBeforeSolve = self.board.current_player
        try:
            result = callAlphabeta(self.board)
        except Exception:
            result = None

        signal.alarm(0)
        self.board.current_player = playBeforeSolve

        best_move_coord = point_to_coord(self.board.bestMove, self.board.size)
        best_move_as_string = format_point(best_move_coord)
        ...
        if(result == None):
            ...
            if result > 0:
                response = "{} {}".format(
                    'b' if self.board.current_player == BLACK else 'w',
                    best_move_as_string)
            # Draw
            elif result == 0:
                response = "draw {}".format(best_move_as_string)
            # Winner is the opponent
            else:
                response = "{}".format(
                    'w' if self.board.current_player == BLACK else 'b')
        return response, result

    def solve_cmd(self, args):
        response, _ = self.solver()
